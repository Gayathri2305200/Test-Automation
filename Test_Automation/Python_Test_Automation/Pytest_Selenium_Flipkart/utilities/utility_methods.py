...
def webdriver_wait_by_name(driver, time, locator):
    wait = WebDriverWait(driver, time).until(ec.presence_of_element_located((By.NAME, locator)))
    return wait


def webdriver_wait_by_xpath(driver, time, locator):
    wait = WebDriverWait(driver, time).until(ec.presence_of_element_located((By.XPATH, locator)))
    return wait


def webdriver_waits(driver, time):
